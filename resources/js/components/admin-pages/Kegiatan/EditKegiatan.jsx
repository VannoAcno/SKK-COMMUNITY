...
const DEFAULT_AVATAR = 'https://ui-avatars.com/api/?name=Edit+Kegiatan&background=FACC15&color=ffffff&size=128';
  ...
  useEffect(() => {
    const fetchKegiatan = async () => {
      try {
        ...
        setAvatarPreview(data.gambar || DEFAULT_AVATAR);
      } catch (err) {
        Swal.fire({
          icon: 'error',
          title: 'Gagal',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Gagal',
        text: err.message || 'Terjadi kesalahan saat memperbarui kegiatan.',
        confirmButtonColor: '#FACC15',
      });
    } finally {
      setLoading(false);
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle className="text-2xl text-[#374151]">Edit Kegiatan</CardTitle>
              </CardHeader>
              <CardContent>
